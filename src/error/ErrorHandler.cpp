...
bool ErrorHandler::generateErrorResponse(Context& ctx) const
{
	...
	else
	{
		content = "<!DOCTYPE html>"
			...
			"100% { background-color: rgb(60, 0, 0); }"
			"}"
			"body::before {"
			"content: '';"
			"position: absolute;"
			...
			"radial-gradient(circle at 50% 70%, rgb(96, 96, 209) 0%, transparent 20%),"
			"radial-gradient(circle at 20% 20%, rgb(255, 0, 0) 0%, transparent 25%),"
			"radial-gradient(circle at 80% 30%, rgb(96, 96, 209) 0%, transparent 15%);"
			"z-index: -1;"
			...
			"  animation: strobe 0.8s infinite alternate-reverse;"
			"}"
			"#beedoo {"
			"  pointer-events: none;"
			"  top: -180px;"
			...
			"  position: absolute;"
			"  opacity: 0.8;"
			"  animation: beedooing 8s infinite alternate-reverse;"
			"}"
			""
			"@keyframes beedooing {"
			"  0% {"
