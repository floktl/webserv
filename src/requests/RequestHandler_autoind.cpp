...
//bool RequestHandler::checkDirectoryPermissions(const RequestBody& req, std::stringstream* response) {
//	if (access(req.requested_path.c_str(), R_OK) != 0) {
//		buildErrorResponse(403, "Permission denied", response, const_cast<RequestBody&>(req));
//		return false;
//	}
//	return true;
//}

//std::vector<RequestHandler::DirEntry> RequestHandler::getDirectoryEntries(RequestBody& req,
...
//void RequestHandler::processDirectoryEntry(const RequestBody& req, struct dirent* dir_entry,
//		std::vector<DirEntry>& entries) {
...
//void RequestHandler::generateAutoindexHtml(std::stringstream* response, const RequestBody& req,
