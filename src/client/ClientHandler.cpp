...
void ClientHandler::handleClientRead(int epfd, int fd)
{
    ...
    {
        Logger::file("[handleClientRead] Invalid fd: " + std::to_string(fd)
    ...
    while (true)
    {
        ...
        if (n == 0)
        {
            Logger::file("[handleClientRead] Client disconnected on fd: "
                         + std::to_string(fd));
            server.delFromEpoll(epfd, fd);
            return;
        }
        ...
        if (n < 0)
        {
            ...
            {
                Logger::file("[handleClientRead] read error: "
        ...
        {
            Logger::file("[handleClientRead] Error parsing request on fd: "
        ...
        {
            Logger::file("[handleClientRead] POST body incomplete. Waiting for more data. "
                         "Received: " + std::to_string(req.received_body.size())
                         + ", Expected: " + std::to_string(req.content_length));
...
void ClientHandler::handleClientWrite(int epfd, int fd)
{
    ...
    {
        Logger::file("[handleClientWrite] Timeout reached for fd: "
                     + std::to_string(fd));
        server.delFromEpoll(epfd, fd);
        return;
    }
    ...
    if (req.state == RequestState::STATE_SENDING_RESPONSE)
    {
        ...
        {
            Logger::file("[handleClientWrite] getsockopt error on fd: "
                         + std::to_string(fd));
            server.delFromEpoll(epfd, fd);
            return;
        }
        ...
        if (!req.response_buffer.empty())
        {
            ...
            if (n > 0)
            {
                ...
                if (req.response_buffer.empty())
                {
                    Logger::file("[handleClientWrite] Response fully sent. Removing fd: "
                                 + std::to_string(fd) + " from epoll.");
                    server.delFromEpoll(epfd, fd);
                }
                else
                {
                    Logger::file("[handleClientWrite] Partially sent response. "
                                 "Still have " + std::to_string(req.response_buffer.size())
                                 + " bytes left to send on fd: " + std::to_string(fd));
            ...
            else if (n < 0)
            {
                if (errno != EAGAIN && errno != EWOULDBLOCK)
                {
                    Logger::file("[handleClientWrite] send() error on fd: "
                                 + std::to_string(fd) + ": "
                                 + std::string(strerror(errno)));
                    server.delFromEpoll(epfd, fd);
                }
